...
app.get("/cornell", (req, res) => {
  res.render("pages/cornell", {
    rutaActual: "/cornell",
  });
});

app.get("/flascard", (req, res) => {
  res.render("pages/flascard", {
    rutaActual: "/flascard",
  });
});
...
app.get("/recuperacion", (req, res) => {
  res.render("pages/recuperacion", {
    rutaActual: "/recuperacion",
  });
});
/**
...
app.post("/procesar-formulario", async (req, res) => {
  ...
  try {
    ...
    const query =
      "INSERT INTO users (gmail, name, passoword, created_at) VALUES (?, ?, ?, ?)";
...
app.post("/procesar-formulario2", (req, res) => {
  ...
  try {
    const query =
      "INSERT INTO estudiantes (gmail, name, password, created_at) VALUES (?, ?, ?,?)";
    connection.query(
      query,
      [gmail,name,password, new Date()],
      (error, result) => {
        if (error) {
          console.error("Error al insertar en la base de datos: ", error);
          res.send("Error al procesar el formulario");
